...
export default {
  ...
  methods: {
    ...
    getData() {
      let self = this;
      self.loading = true;
      console.log(self.queryForm.week);
      let date = self.queryForm.week;
      var weekday = date.getDay() || 7; //获取星期几,getDay()返回值是 0（周日） 到 6（周六） 之间的一个整数。0||7为7，即weekday的值为1-7
      date.setDate(date.getDate() - weekday + 1);//往前算（weekday-1）天，年份、月份会自动变化
      let startDate = date;
      let endDate = new Date(self.queryForm.week.getTime() + (6 * 24 * 60 * 60 * 1000));
      self.queryForm.startTime = [startDate.Format("yyyy-MM-dd") + " 00:00:00", endDate.Format("yyyy-MM-dd") + " 23:59:59"];
