    ...
    <el-dialog title="新增课程" :visible.sync="dialogFormVisible" :close-on-click-modal=false v-loading="loadingForm">
      ...
      <el-form :model="form" ref="ruleForm2" :style="formActive!=2?'display:none':''">
        ...
        <el-form-item :label-width="formLabelWidth" v-for="(chargeStandard, index) in form.chargeStandard" :key="index" style="margin-bottom:0px;">
          <el-form-item v-if="form.standardType==2" style="display:inline-block;width:100px;margin-bottom:5px;" :key="index" :prop="'chargeStandard.' + index + '.minHourse'" :rules="[
          ...
          <el-form-item v-if="form.standardType==2" style="display:inline-block;width:100px;margin-bottom:5px;" :prop="'chargeStandard.' + index + '.maxHourse'" :rules="[
                        { required: true, message: '必填项'},
                        { type: 'number', message: '必须为数字值'}]" size="mini">
            <el-input v-model.number="chargeStandard.maxHourse" placeholder="最大课时"></el-input>
          </el-form-item>

          <el-form-item v-if="form.standardType==1" style="display:inline-block;width:220px;margin-bottom:5px;" :prop="'chargeStandard.' + index + '.minHourse'" :rules="[
                        { required: true, message: '必填项'},
                        { type: 'number', message: '必须为数字值'}]" size="mini">
