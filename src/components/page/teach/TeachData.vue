  ...
  <div class="table" style="height: 90%;">
    ...
    <el-container style="height: 100%; border: 1px solid #eee">
      ...
      <el-aside width="200px" style="background-color:#fff9e7">
        <p v-for="(space, levelIndex) in parameterData" @click="parameterChange(levelIndex,space)" :class="{active: activeIndex==levelIndex}">{{space.name}}</p>
      </el-aside>
      <el-container>
        <!-- 公共参数表格信息 -->
        <el-table :data="tableData" stripe v-loading="loading" border style="width: 100%">
          <el-table-column label="创建人" prop="createUser.name">
          </el-table-column>
...
<script>
import { findBranchParameterByType, findBranchParameterValueAll, createBranchParameterValue } from "../../api/api";
export default {
  data() {
    return {
      formTitle: "",
  ...
  methods: {
    ...
    getData() {
      let self = this;
      self.loading = true;
      findBranchParameterByType(2).then(data => {
        if (data.code == 200) {
