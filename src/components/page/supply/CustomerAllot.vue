<template>
  <div class="table">
    ...
    <el-table :data="tableData" stripe v-loading="loading" @sort-change="handSortChange" border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="创建人" sortable="custom" prop="ownerName" v-if="$isAuthority('show-all-customer')">
      </el-table-column>
    ...
    <allot-form @saveSuccess="getData" :students="checkedData" :dialog-form-visible.sync="dialogOwnerVisible"></allot-form>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      if (!falg) self.queryForm.createUserId = self.$user().id; //查询全部用户权限
      getCustomerList(
        self.cur_page,
        self.page_size,
        self.queryForm
  ...
  components: { CustomerForm, CustomerView, ContactForm, InviteForm, SchoolTree, AllotForm } //注入组件
};
</script>
